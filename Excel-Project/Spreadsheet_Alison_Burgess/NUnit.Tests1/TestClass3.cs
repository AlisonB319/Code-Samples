...
namespace NUnit.Tests1
{
    ...
    public class TestClass
    {
        ...
        public void TestLoad()
        {
            ...
            var fileStream = new FileStream(path + "../../../../c9.xml", FileMode.Open);
            spreadsheet.LoadSpreadSheet(fileStream);

            AbCell cell1 = spreadsheet.GetCell(8,2); // C9
            Assert.AreEqual(cell1.Text, "431");
        ...
        {
            SpreadSheet ss = new SpreadSheet(50, 26);
            AbCell cell = ss.GetCell(0, 0);
            cell.Text = "432";
            cell.Color = 0x0000f0ff;
            cell = ss.GetCell(3, 2);
            cell.Text = "=A1*10-4";
            string path = Path.GetTempPath() + "testingSave.xml";
            using (FileStream toSave = new FileStream(@path, FileMode.Create))
            {
                ss.SaveSpreadSheet(toSave);
            }
            Assert.That(File.Exists(@path));
            Assert.AreEqual(cell.Text, "=A1*10-4");
        }
        ...
        public void TestCir()
        {
            ...
            SpreadSheet ss = new SpreadSheet(50, 26);
            AbCell cell = ss.GetCell(0, 0);
            // check for a bad self ref
            cell.Text = "=A1";
            cell = ss.GetCell(0, 0);
            Assert.AreEqual(cell.Value, "!(bad refrence)");

            // check for a circ ref
            cell.Text = "=B1";
            AbCell B1cell = ss.GetCell(0, 1);
            B1cell.Text = "=B2";
            AbCell B2cell = ss.GetCell(1, 1);
            B2cell.Text = "=A2";
            AbCell A2cell = ss.GetCell(0, 1);
            A2cell.Text = "=A1";
            Assert.AreEqual(A2cell.Value, "!(bad refrence)");

            cell.Text = "=B1";
            B1cell = ss.GetCell(0, 1);
            B1cell.Text = "=B2";
            B2cell = ss.GetCell(1, 1);
            B2cell.Text = "123";
            Assert.AreEqual(cell.Value, "123");
            Assert.AreEqual(B1cell.Value, "123");
